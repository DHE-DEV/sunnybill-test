            ...
            <div class="fixed inset-0 z-[9999] overflow-y-auto"
                ...
                <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center" style="z-index: 9999 !important;">
                    ...
                    <div class="relative inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all" style="width: 90vw !important; max-width: 800px !important; z-index: 9999 !important; padding: 20px !important; max-height: 80vh !important;">
                        ...
                        <!-- History List -->
                        <div class="pt-4 space-y-4" style="max-height: 500px !important; overflow-y: auto !important;">
                                ...
                                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 shadow-sm">
                                    <div class="flex items-start justify-between mb-2">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-sm text-gray-500 ml-3">&nbsp;&nbsp;&nbsp;{{ $historyEntry->created_at->format('d.m.Y H:i') }}</span>
                                        </div>
                                        <span class="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-800">
                                            {{ ucfirst($historyEntry->action) }}
                                        </span>
                                    </div>
                                    
                                    @if($historyEntry->action === 'created')
                                        <div class="text-sm text-gray-700">
                                            <strong>Aufgabe erstellt</strong>
                                        </div>
                                        @if($historyEntry->description)
                                            <div class="text-sm text-gray-600 mt-1">{{ $historyEntry->description }}</div>
                                        @endif
                                    @elseif($historyEntry->action === 'field_changed')
                                        <div class="text-sm text-gray-700">
                                            <strong>{{ ucfirst($historyEntry->field_name) }} geändert</strong>
                                        </div>
                                        <div class="text-sm text-gray-600 mt-1">
                                            <span class="line-through text-red-600">{{ $historyEntry->old_value }}</span>
                                            <span class="mx-2">→</span>
                                            <span class="text-green-600 font-medium">{{ $historyEntry->new_value }}</span>
                                        </div>
                                    @elseif($historyEntry->action === 'note_added')
                                        <div class="text-sm text-gray-700">
                                            <strong>Notiz hinzugefügt</strong>
                                        </div>
                                        @if($historyEntry->description)
                                            <div class="text-sm text-gray-600 mt-1">{{ $historyEntry->description }}</div>
                                        @endif
