...
class CreateAppToken extends CreateRecord
{
    ...
    protected function handleRecordCreation(array $data): AppToken
    {
        ...
        // Speichere für Modal-Anzeige
        $this->createdToken = $appToken;
        $this->createdTokenPlainText = $plainTextToken;

        // Einfache Bestätigungsbenachrichtigung
        Notification::make()
            ->title('Token erfolgreich erstellt')
            ->body("Token '{$appToken->name}' wurde erfolgreich erstellt. Das Modal mit den Token-Details wird gleich angezeigt.")
            ->success()
            ->send();
    ...
    {
        // Modal mit Token-Details anzeigen
        if ($this->createdToken && $this->createdTokenPlainText) {
    ...
    {
        return [
            Action::make('show_created_token')
                ->label('Token-Details anzeigen')
                ->modalHeading('🔑 Neuer API-Token erstellt')
                ->modalSubheading(fn() => $this->createdToken ? "Token: {$this->createdToken->name}" : 'Token-Details')
                ->modalWidth('4xl')
                ->modalContent(function () {
                    if (!$this->createdToken || !$this->createdTokenPlainText) {
                        return new HtmlString('<p>Keine Token-Details verfügbar.</p>');
                    }

                    // Generiere QR-Code für den Token
                    $qrCodeService = new AppTokenQrCodeService();
                    $qrCodeBase64 = $qrCodeService->generateSimpleTokenQrCode($this->createdTokenPlainText);
                        ...
                        <div class='space-y-6'>
                            ...
                            <div class='bg-blue-50 p-4 rounded-lg border border-blue-200'>
                                ...
                                <div class='grid grid-cols-2 gap-4 text-sm'>
                                    <div><strong>Name:</strong> {$this->createdToken->name}</div>
                                    <div><strong>App-Typ:</strong> {$this->createdToken->app_type_label}</div>
                                    <div><strong>Berechtigungen:</strong> " . implode(', ', $this->createdToken->abilities_labels ?? []) . "</div>
                                    <div><strong>Gültig bis:</strong> {$this->createdToken->expires_at->format('d.m.Y H:i')}</div>
                                </div>
                            </div>
                            ...
                            <div class='bg-gray-50 p-6 rounded-lg border'>
                                <div class='flex justify-between items-center mb-4'>
                                    ...
                                    <button onclick='copyTokenToClipboard(\"{$this->createdTokenPlainText}\")' 
                                            id='copy-token-btn'
                ...
                ->visible(fn() => $this->createdToken !== null)
                ->extraAttributes([
                    'wire:key' => 'token-modal-' . ($this->createdToken?->id ?? 'none'),
                    'x-on:show-token-modal.window' => '$wire.mountAction(\'show_created_token\')'
                ]),
        ];
    }
